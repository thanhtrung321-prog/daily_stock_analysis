...
- [新功能] `REPORT_LANGUAGE` 新增 `vi` 越南语输出，覆盖主分析 Prompt、报告模板、Agent 决策语言提示、配置选项与越南语快速设置文档。
- [修复] 优化 WebUI 登录页、首页工具栏、问股消息区与设置页的移动端响应式布局，避免小屏横向溢出和控件挤压。
- [修复] WebUI 问股策略选择器在越南语模式下显示越南语策略名与说明，避免内置技能名称继续露出中文。
- [改进] 标准化 WebUI 工作台桌面布局，统一导航、命令栏、历史侧栏和主内容区的间距与视觉层级。
