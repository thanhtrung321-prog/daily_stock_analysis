...
- [新功能] A 股大盘复盘推送新增“热门板块 / 热门股票”附加摘要，按板块涨跌幅 Top N 与个股涨跌幅优先、成交额次排序追加展示；任一数据源缺失时仅跳过对应区块，不阻断原复盘输出；`MARKET_REVIEW_REGION=both` 维持 A 股 + 美股语义，不扩展为港股复盘，也不涉及 LLM/provider/Base URL/模型默认值或旧配置迁移。
