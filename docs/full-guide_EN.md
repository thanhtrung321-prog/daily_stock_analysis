...
> - The config read paths for `LITELLM_CONFIG`, `LLM_CHANNELS`, and legacy `GEMINI_*` / `OPENAI_*` / `ANTHROPIC_*` variables are unchanged; this feature does not silently migrate, delete, or rewrite legacy keys.
