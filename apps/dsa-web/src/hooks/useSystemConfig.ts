...
export function useSystemConfig() {
  ...
  const save = useCallback(async (): Promise<SaveResult> => {
    if (!hasDirty) {
      setToast({ type: 'success', message: 'Hiện không có thay đổi cần lưu.' });
      return { success: true, message: 'Hiện không có thay đổi cần lưu' };
    }
    ...
    try {
      ...
      if (!validateResult.valid) {
        setSaveError(createParsedApiError({
          title: 'Cấu hình chưa vượt qua kiểm tra',
          message: 'Vui lòng sửa lỗi trong biểu mẫu trước khi lưu.',
          rawMessage: 'Cấu hình chưa vượt qua kiểm tra, vui lòng sửa lỗi trong biểu mẫu.',
          category: 'http_error',
        }));
        setRetryAction('save');
        return {
          success: false,
          message: 'Cấu hình chưa vượt qua kiểm tra',
      ...
      const warningText = updateResult.warnings?.length
        ? `; cảnh báo: ${updateResult.warnings.join('; ')}`
        : '';
      setToast({ type: 'success', message: `Cấu hình đã cập nhật${warningText}` });
      return { success: true };
    } catch (error: unknown) {
      if (error instanceof SystemConfigValidationError) {
        ...
        setSaveError(createParsedApiError({
          ...
          message: `${error.message}, vui lòng tải lại cấu hình trước.`,
      ...
      setRetryAction('save');
      return { success: false, message: 'Lưu thất bại' };
    } finally {
      setIsSaving(false);
    }
  }, [
